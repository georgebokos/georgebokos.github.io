...
SAFE_B = 380     # κάτω
SAFE_R = 170     # δεξιά
TOP    = 350     # κάτω από υπογραφή, γλώσσες και σήμα Google Play
...
def hook_words(el=True):
    ...
    # ΚΑΜΙΑ ΤΙΜΗ στο πρώτο καρέ. Δοκιμάστηκε και σε δύο εκδοχές («2.25€ η
    # μερίδα», και με ετικέτα «ΚΟΣΤΟΣ ΥΛΙΚΩΝ» από πάνω) — και στις δύο ένα
    # νούμερο σε ευρώ πάνω σε φωτογραφία φαγητού διαβάζεται ως τιμοκατάλογος
    # καταστήματος. Στη θέση του μπαίνει το πλήθος υλικών: πληροφορία χρήσιμη
    # για τον μάγειρα, αδύνατο να παρεξηγηθεί ως πώληση.
    return {'time':'και έτοιμο' if el else "and it's ready",
            'ing' :'υλικά. Τίποτα άλλο.' if el else "ingredients. That's all.",
            'ing2':'υλικά χρειάζεσαι' if el else 'ingredients needed',
            'cal' :'θερμίδες η μερίδα' if el else 'calories per serving',
            'srv' :'μερίδες βγάζει η συνταγή' if el else 'servings from one batch',
...
def pick_hook(m, ings, steps, rid, el=True):
    ...
    opts = []
    # Κάθε αριθμός μπαίνει ΜΟΝΟ όταν παίζει υπέρ του πιάτου: «100′ και έτοιμο»,
    # ή «380 θερμίδες» πάνω σε γλυκό, διώχνουν τον θεατή αντί να τον κρατήσουν.
    ...
    # Πάντα διαθέσιμο: λίγα υλικά υπόσχονται απλότητα, πολλά απλώς ενημερώνουν.
    opts.append((None, str(len(ings)),
                 W_['ing'] if len(ings) <= 7 else W_['ing2']))
    # Οι θερμίδες πουλάνε φαγητό, όχι γλυκό.
    if m.get('cal') and m['cal'] <= 400 and not sweet:
        opts.append((None, str(m['cal']), W_['cal']))
    # Λίγα βήματα: υπόσχεση ότι δεν είναι δύσκολο. Ήταν μόνο εφεδρικό και
    # σπάνιζε — τώρα μπαίνει κανονικά στην κλήρωση.
    if len(steps) <= 7:
        opts.append((None, str(len(steps)), W_['step']))
    # Πόσες μερίδες βγάζει: μετράει μόνο όταν είναι πολλές — «4 μερίδες» δεν
    # εντυπωσιάζει κανέναν, «40 μερίδες» ναι.
    if m.get('srv') and m['srv'] >= 6:
        opts.append((None, str(m['srv']), W_['srv']))
    if opts:
...
def build(rid, lang='el'):
    ...
    def stamp(fr, d):
        ...
        # Το σήμα του Google Play, επίσης σε κάθε καρέ: αναγνωρίζεται χωρίς
        # διάβασμα και λέει αμέσως πού βρίσκεται η εφαρμογή — για τον ξένο
        # θεατή είναι το μόνο στοιχείο που δεν χρειάζεται μετάφραση.
